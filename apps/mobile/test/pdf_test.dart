...
void main() {
  ...
  testWidgets('PDF opens, navigates and closes the renderer', (tester) async {
    ...
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(documentChannel, (call) async {
      calls.add(call.method);
      if (call.method == 'openPdf') return 2;
      if (call.method == 'renderPdf') return png;
      return null;
    });
    await tester.pumpWidget(MaterialApp(home: PdfViewerPage(bytes: Uint8List(0), title: 'teste.pdf')));
  ...
  testWidgets('export uses existing decrypted attachment and cancellation is not success', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: DocumentDetailsPage(store: store,
      item: VaultDocument(id: '1', type: 'Passaporte', title: 'Teste', createdAt: DateTime(2026),
        attachmentFile: 'existing.karta', attachmentName: 'teste.pdf', attachmentMime: 'application/pdf'))));
    await tester.pumpAndSettle();
