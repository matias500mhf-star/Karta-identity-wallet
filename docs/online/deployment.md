...
3. Use a fresh database for the first deployment. Start command applies Prisma migrations and then the API. For an EXISTING database, review migration history first: the old `0002_add_document_shares` duplicate was removed because it sorted before the initial tables and the initial migration already creates that table, its indexes and foreign keys. If that duplicate was already recorded or failed in an existing database, reconcile its migration history with a database administrator before deployment. The initial migration is unchanged. Never reset a database to resolve this.
